...
int initialize_port(ahci_port_t* port, uint32_t index, HBA_PORT* port_desc)
{
...
int ahci_port_identity(ahci_port_t *port, char* buffer)
{
	...
	HBA_COMMAND_TABLE *cmdtbl = (HBA_COMMAND_TABLE*)vmm_get_virtual_address(((uintptr_t)cmdheader->ctdba_up << 32) | cmdheader->ctdba_low);
...
int ahci_port_read_lba(ahci_port_t* ahci_port, uint64_t start, uint64_t count, char *buf)
{
	int rc = 0;
	...
	if (reqd_bytes > ahci_port->read_buffer_size)
	{
		if ((ahci_port->read_buffer_phys != 0) && (ahci_port->read_buffer != 0))
		{
			...
			pmm_free_pages((void*)ahci_port->read_buffer_phys, ahci_port->read_buffer_pages);
		}

		ahci_port->read_buffer_phys = (uintptr_t) pmm_alloc(reqd_bytes, &ahci_port->read_buffer_pages);
		//printk("New buffer %p\n", ahci_port->read_buffer_phys);
...
int ahci_port_write_lba(ahci_port_t* ahci_port, uint64_t start, uint64_t count, const char *buf)
{	
	int rc = 0;
	size_t reqd_bytes = count * 512;
